...
func TestDoMulti_CapsAtMaxParallel(t *testing.T) {
	...
	h.DoMulti(context.Background(), fns)
	if count.Load() > 2 {
...
func TestOnHedge_Called(t *testing.T) {
	...
	h.Do(context.Background(), func(ctx context.Context, hc HedgeController) (string, error) {
		time.Sleep(50 * time.Millisecond)
		return "ok", nil
	})
...
func TestStats(t *testing.T) {
	h := New[string](WithMaxParallel(2), WithDelay(5*time.Millisecond))
	h.Do(context.Background(), func(ctx context.Context, hc HedgeController) (string, error) { return "ok", nil })
	h.Do(context.Background(), func(ctx context.Context, hc HedgeController) (string, error) { return "", errors.New("fail") })
