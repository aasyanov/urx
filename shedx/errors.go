...
var (
	...
	// because the shedder is overloaded for its priority. The wrapped error
	// carries the request priority. Safe to compare with == or [errors.Is].
	ErrRejected = errors.New("shedx: request shed")

	// ErrClosed is returned by [Execute] and [Acquire] after [Shedder.Close]
	// has been called. Safe to compare with == or [errors.Is].
	...
	// cancelled (or its deadline has expired) at admission time, so no in-flight
	// slot is consumed and fn is never invoked. The joined error carries
